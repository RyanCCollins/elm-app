...
function* isAuth(next) {
  ...
  const {valid} = verifyToken(JWT_PW, token);
  if (valid) {
    yield next;
  } else {
...
function* addRunner() {
  ...
  try {
    ...
    this.body = 'Error saving the runner!';
    this.status = 400;
  }
}
  ...
  db.find({})
    .then( (runners) => {
      const promises = runners.map ( (runner) => {
        ...
        const lastMarkerTime = minute > -1 ?
          (new Date( startTime + markerMinutes * 60000)).getTime() :
          0;
        const updatedRunner = Object.assign(runner,
          {lastMarkerDistance, lastMarkerTime});
